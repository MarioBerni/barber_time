...
import 'stylized_button_animations.dart'
    show
        StylizedButtonAnimationController,
...
///
/// Permite crear botones con diferentes estilos
...
class _StylizedButtonState extends State<StylizedButton> {
  ...
  final GlobalKey<StylizedButtonAnimationControllerState> _animationKey =
      GlobalKey();
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.showGlowEffect && widget.type == StylizedButtonType.primary) {
      buttonContent = Stack(
        children: [
          ...
          AnimatedOpacity(
            opacity: _isPressed
                ? (_animationKey.currentState?.opacityValue ?? 0.0) * 0.5
                : 0.0,
    ...
    // Construir el botón completo con GestureDetector
    // para manejar los eventos de presión
  ...
  Widget _buildButtonContent(BuildContext context) {
    ...
    if (widget.icon != null) {
      iconWidget = Icon(
        widget.icon,
        size: 18.0, // Reducido de 20.0 para mantener proporción
        // con el nuevo tamaño
    ...
    if (iconWidget != null) {
      if (widget.iconOnRight) {
        children.addAll([textWidget, context.horizontalSpaceXS, iconWidget]);
      } else {
        children.addAll([iconWidget, context.horizontalSpaceXS, textWidget]);
      }
    } else {
